...
export function ListingsSection({ listings }: ListingsSectionProps) {
  ...
  const currentRoute = (pages: string[]) => {
    const currentPath = useLocation().pathname.slice(1).toLowerCase();
  ...
  return (
    <section className="bg-fog px-6 py-10 md:px-12 lg:px-16">
      <div className="mx-auto max-w-7xl">
        {!currentRoute(["favorites", "search"]) && <Reveal className="mb-10 flex items-end justify-between gap-6">
          <div>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {listings.map((listing, index) => {
            const slug = listing.title.toLowerCase().replace(/[^a-z0-9]+/g, "-");
            const isFav = isFavorite(slug);

            return (
              <Reveal key={listing.title} delay={50 * ((index % 4) + 1)}>
                <Link to={`/spaces/${slug}`} className="block h-full outline-none focus-visible:ring-2 focus-visible:ring-clay focus-visible:ring-offset-2 rounded-[20px]">
                  <article className="group cursor-pointer overflow-hidden rounded-[20px] bg-white transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_20px_50px_rgba(26,23,19,0.12)] h-full">
                    <div className="relative aspect-[4/3] overflow-hidden">
                      ...
                      >
                        <HeartIcon filled={isFav} className={`h-4.5 w-4.5 ${isFav ? "text-rust" : "text-ink/70"}`} />
                      </button>
